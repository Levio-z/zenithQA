...
import com.zenith.qa.mapper.ScoringResultMapper;
...
public class ScoringResultServiceImpl extends ServiceImpl
    ...
    public void validScoringResult(ScoringResult scoringResult, boolean add) {
        ...
        if (StringUtils.isNotBlank(resultName)) {
            ThrowUtils.throwIf(resultName.length() > 128, ErrorCode.PARAMS_ERROR, "结果名称不能超高 128");
        }
        if (appId != null) {
    ...
    public Page<ScoringResultVO> getScoringResultVOPage(Page<ScoringResult> scoringResultPage, HttpServletRequest request) {
        ...
        // region 可选
        // 1. 关联查询用户信息
        Set<Long> userIdSet = scoringResultList.stream().map(ScoringResult
